...
def choosen_student_submitting_answers(request, id, id2):
    ...
    for a in answerClosed:
        p = StudentAnswer(student_id=students_taking_exam.id, answer_id=a, is_selected=1,
                          points_for_answer=points[b].is_correct)
        p.save()
        b += 1

    b = 0
    for a in answerOpen:
        p = StudentOpenQuestion(student_id=students_taking_exam.id, open_question_id=answerOpenQuestionId[b],
                                is_graded_by_teacher=0,
